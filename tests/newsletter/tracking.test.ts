...
describe("newsletter click tracking", () => {
  it("wraps composer-authored web links and records recipient attribution", () => {
    const html = [
    ...
    const [trackedHref, siteHref] = [...sent.matchAll(/href="([^"]+)"/g)].map(
      (match) => match[1],
    );
    ...
    expect(
      resolveRedirect(tracked.pathname.split("/").pop()!, tracked.searchParams.get("s")!),
    ).toBe("https://example.com/article?source=newsletter&campaign=weekly");

    const site = new URL(siteHref!);
    expect(resolveRedirect(site.pathname.split("/").pop()!, site.searchParams.get("s")!)).toBe(
  ...
  it("does not double-wrap links that already use the redirector", () => {
